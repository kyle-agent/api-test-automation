...
| `formal/resources/` | **The M5 resource-task model — the formal destination knowledge converges into.** One YAML per service (283 nodes / 60 YAML files = 59 services + `_groups.yaml` — live count `python knowledge/formal/validate.py`; codes `<cat>-<group>-<resource>`): per-resource `requires` (incl. `one_of`/`count`/`credential`), validated create-body templates, options, capture/ready/delete, `provenance` + live-run citations in `notes`. `regression/scenarios/composer.py` compiles it into engine lifecycles (`gen-*`/`bundle-*`) — long-term this layer **generates** the engine data instead of mirroring it. Validated by the same `validate.py` (R1 checks). See `docs/RESOURCE-MODEL-PLAN.md`. |
...
| [`resource-dependencies.md`](resource-dependencies.md) | Attach/detach test patterns + creation-order DAG (2026-06-09 스냅샷 — 갭 표는 당시 기준; 현행 정본은 formal 모델/lifecycles). |
| [`domain-constraints.md`](domain-constraints.md) | Confirmed account constraints: quotas, CIDR rules, per-lifecycle /20 allocation, retry/400-as-409 classes. |
| [`vpc-scheduling-strategy.md`](vpc-scheduling-strategy.md) | VPC 슬롯 레인 플레이북 (machine-readable: `dependencies.json:vpc_schedule`). |
