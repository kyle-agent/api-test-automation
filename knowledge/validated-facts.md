...
## WAF 행동 차단 실측 — 재시도 사다리 설계 상한 (2026-07-16, run 7a26)

- 동일 바디 POST를 20s 간격 반복(403 사다리)하자 **보안 게이트웨이가 417 +
  HTML 'Request Rejected'(Support ID 3232170405160507975)로 차단** — 앱 레이어
  아님(JSON 에러 아님). F5 계열 WAF 차단 페이지 형식.
- 규칙: **재시도 사다리는 동일-바디 뮤테이션 반복을 삼가라** — 간격 ≥30s,
  횟수 한 자리수. 구조 원인(쿼터/상태)은 사다리가 아니라 레인 분리/게이트로
  풀 것. 417+HTML 응답을 보면 WAF 차단으로 판정 (엔진 재시도 대상 아님).

## 417 정정: 원인은 오프닝 버스트 (2026-07-16, 오너 판정 + 실측)

- run 7a26 실측: t=0 병렬 디스패치 첫 적용 런에서 **첫 60초에 lifecycle 80개
  동시 출발** — scr create 417은 사다리보다 이 오프닝 버스트에 WAF 레이트
  규칙이 걸린 것 (오너: "초반에 요청이 몰리면서 417 난거 같아" — 정확).
...
## notfound 프로브 공정성 수리 (오너 반론 2026-08-20)

- **반론 1 (형식 400) 타당**: 프로브의 부재-id가 대시 UUID 한 종류였는데 SCP
  id는 32-hex(대부분)/대시 UUID(ports 등) 혼재 — 32-hex 서비스엔 형식 위반
  400이 정당해 판정 오염. → 두 포맷 모두 발사, 하나라도 404면 합격, 응답
  발췌(excerpt) 동봉으로 400 원인 분류 가능하게 수리. 기존 88건 중 일부는
  재측정 시 해소될 수 있음 (다음 📐 클릭에서 재분류).
...
## 공정성 재측정 최종 (2026-08-20 저녁 프로브 런)

- 이중 id-포맷 notfound: ok_404 192→**199** (형식-400 오탐 7건 구제),
  non-404 83 잔존 (그중 403 계열은 부록行). pagination 문서화 게이트:
...
## vpc 1.4 핀 → VpcCreateRequestV1Dot4 `zone_type` 필수 (2026-09-21, run 20260921-094658-d961)

- 9/18 버전 핀 재생성으로 vpc 제품 핀이 1.3→1.4가 되자 `POST /v1/vpcs`가 즉사: V1Dot4 모델은
  **`zone_type` 필수**(enum PUBLIC/PRIVATE/ACCELERATED), `zones` 선택(PRIVATE/ACCELERATED는 1개 필수).
