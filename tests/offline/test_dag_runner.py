...
def _genuine_time_sleep(monkeypatch):
    ...
    task durations, overlap windows) to ~0 and breaks the concurrency
    assertions — pass alone, fail in the full offline run. Pin a genuine sleep
    per-test here; monkeypatch restores the outside state afterwards. Proper
    fix belongs in verify_clean (stub sleep only inside scan_owned's
    try/finally, like _delete/_wait_gone)."""
