...
Present the brief. Do not write `drafts/BRIEF.md` yet.
...
Save the approved brief to `drafts/BRIEF.md` in the project root (or cwd if no project root detected).

Print:
```
Brief saved to drafts/BRIEF.md.

Next: read drafts/BRIEF.md and implement it (same session or a new one).
```
...
| Save drafts/BRIEF.md | Analyze existing code in depth (quick scan of relevant files only) |
...
5. **Approval gate is mandatory**: Save `drafts/BRIEF.md` only after explicit user approval. Violation → An unreviewed brief gets saved, and implementation starts from a wrong spec.
...
- **drafts/BRIEF.md**: Saved to project root (or cwd). Sections: Goal / Scope IN / Scope OUT / Constraints / Exit Criteria / Risk Flags.
...
- **Brief is a living document**: If scope changes during implementation, update drafts/BRIEF.md first, then continue implementation to the updated spec. Do not expand scope without updating the brief.

---

## Truthful Reporting

When reporting completion after creating or updating drafts/BRIEF.md:
