...
name: retro
description: "Milestone retrospective. After a feature or sprint completes, extract what went wrong, what went right, and what patterns emerged. Write actionable lessons to docs/lessons.md."
depends_on: []
tags: [learning, meta]
version: "1.0.0"
triggers:
...
For each pattern (and significant one-off issues), write to `docs/lessons.md`:
...
- If `docs/lessons.md` does not exist, create it now before proceeding:
  ```markdown
  # Lessons
  <!-- conf scale: 0.3 (tentative) → 0.5 (moderate) → 0.7+ (verified) -->
  <!-- Format: ### [YYYY-MM-DD] Title / > conf: X · seen: YYYY-MM-DD · obs: N / body -->
  ```
- Check for duplicates first: `Grep` docs/lessons.md for similar patterns before writing
...
| [READ] Read docs/lessons.md for duplicate check | Delete existing lessons |
| [GREP] Search for duplicate lessons before writing | Rewrite or restructure lessons.md |
| [EDIT] Append new entries to docs/lessons.md | Create new files outside lessons.md |
...
| `missing_data` | docs/lessons.md doesn't exist | Create file with header, then proceed |
...
2. **Duplicate check first**: Before writing any new lesson, `Grep` docs/lessons.md for the same pattern. Writing duplicates inflates obs counts and makes lessons harder to maintain. Violation → lessons.md becomes inconsistent.
