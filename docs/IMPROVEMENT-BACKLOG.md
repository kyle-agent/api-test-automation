...
| IB-035 | debt | Direct Connect 선행 Security Group이 cross_constraints엔 문서화됐으나 `cross-service.yaml: direct-connect.requires=[vpc]`에 미반영. Track N | requires에 security-group 추가(1줄; SG는 시나리오에서 이미 생성, 리스크 낮음). composer closure 변화라 게이트 확인 | S | **done (Q: cross-service.yaml graph 레이어에 추가, R1 0err)**. ⚠️NOTE: composer는 `resources/*.yaml`을 쓰므로 이 변경은 의존-그래프 시각화만 정리(DC 합성 closure 무변, +1 divergence 권고는 R3-reconcile류). DC 합성에 SG를 넣으려면 `resources/networking__direct-connect.yaml` 후속 필요 |
...
| IB-041 | platform | **라이브 run이 docs→VALIDATED 승급을 못 함(검증 병목)** — heavy/ADOPT lifecycle의 create step이 soft/optional(tolerant expect, 4xx 허용)이라 lifecycle "pass"가 실제 2xx를 증명 못 함 + `verified_endpoints.json` 부재. run #108: 9 lifecycle 통과했으나 masked-defect 규칙상 승급 0 (Track R). 즉 heavy run은 "안 깨짐"만 확인, VALIDATED 숫자 못 올림 | publish/엔진이 `reports/results/observations.jsonl`에서 **실제 2xx 받은 endpoint를 `data/baselines/verified_endpoints.json`로 산출·커밋** → 승급에 endpoint별 증거 부여(masked-defect-safe). 또는 승급 목표 노드의 create를 strict화(soft→strict). 그 후 재-run이 VALIDATED를 실제 전진 | M | **in-progress (V1: tools/derive_verified.py + 회귀테스트 + publish 배선 done; dashboard-data에 verified_endpoints_evidence.json 누적 발행. 다음 라이브 run이 증거 생성 → 그걸로 승급. 승급규칙: 노드 create endpoint_key ∈ verified)** |
| IB-042 | platform | **composer가 create step의 `optional`/`expect_status`/`note`를 passthrough 안 함** — `composer.py:809`이 create expect를 `[200,201,202]`로 하드코딩. 함의 ①모델의 create-tolerate(예: PF-21 volume-transfer Fix A)가 합성 시나리오에 반영 안 됨 ②합성 create는 strict 2xx → **합성 lifecycle 통과는 승급 증거로 유효**(IB-041 검증 루프는 hand-written보다 gen-* 우선). Track W1 | composer가 `create.optional/expect_status/note`를 읽어 합성 step에 전달; 그 후 PF-21 tolerate가 합성에도 적용 | M | open |
| IB-043 | coverage | mysql `set-parameter-values` 400 value_error — `{id,new_value,old_value}` 삼중쌍은 맞으나 lookup(`mysql-parameter`)이 `applied_value` 하나만 캡처해 distinct `new_value`를 못 줌(old_value=new_value=동일 토큰). 현재 optional+[200,202,400] gated(Y2). 동일 패턴이 epas/mariadb/pg/ss param에도 적용 가능 | applied_value 별도 캡처 + 유효 범위 내 distinct new_value 도출(또는 known-safe modifiable param 고정) → param-update write 실검증 | M | open |
| IB-044 | platform | light-run 로그에 **coverage summary 줄(cov_op/cov_getid/verified count) 누락** — Wave A triage가 cov 수치를 못 읽음(heavy-run엔 있음). Track triage | light CRUD 경로에도 coverage summary emit(dashboard/build 또는 run step) → light-run triage가 cov 델타 직접 확인 | S | open |
| IB-045 | platform | **evidence endpoint_key 불일치** — 일부 관측(read/probe step)이 catalog 키(`cat/svc/op`)가 아니라 `lifecycle:step` 형식으로 기록돼 `verified_endpoints.json`에 service-prefix 없는 키가 섞임 → service-disambiguation 실패(quick-query-list 오승급 → Watcher Z2가 적발·revert). Track Z2 | 관측 기록 시 endpoint_key를 항상 catalog 키로 정규화, 또는 derive_verified가 lifecycle:step→catalog endpoint_key 매핑 후 산출 → 승급 disambiguation 신뢰 | M | open |
