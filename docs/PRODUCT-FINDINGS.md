...
| PF-21 | compute/virtualserver · `POST /v1/volume-transfer` (create-volume-transfer, gen-wave2-volume) | 500 `InternalServerError` on a complete-looking body (name + real volume_id) — a NEW 5xx-on-create; req-bc27a3b6 | 27540589368 (req-bc27a3b6) | **UNCONFIRMED — pending 2nd repro** (1 sighting; not baselined per no-blind-baseline rule) | candidate — NOT in `known_issues.json`. Action for the virtualserver/compose owner (out of this agent's file scope): make the composed `create-volume-transfer` step tolerate 500 with note "PF-21 candidate" so the job stops failing pending a confirming repro; promote to baselined Product Bug only on a 2nd sighting. |
