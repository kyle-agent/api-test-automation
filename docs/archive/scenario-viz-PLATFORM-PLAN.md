---
status: superseded (poc/scenario-viz 은퇴와 함께 아카이브 — 확정 IA 정본은 docs/working/plans/PLATFORM-IA-DIRECTION.md §확정IA)
for: all
...
## P4 상태 — Report 단계별 수행시간 (2026-06-15)
- `graph_export.build_report(model, observations)`: 런의 `observations.jsonl`(호출별
  `elapsed_ms`)을 **노드 create endpoint(method+path)로 매칭** → 노드별
  pass/fail·http·평균 elapsed. 순수 함수, 합성 데이터로 검증(offline test 4건).
- 정적 `report.html`: 폐포 그래프를 결과로 색칠(pass/fail/미측정) + **단계별 수행시간 표** +
  순차 합계 vs **레벨 병렬 wall-clock**(Σ단계별최장) + 가속배수. 런 데이터 없으면 배너로 안내.
- CI: `graph_export "$dd/catalog" reports/results/observations.jsonl` — 매 런 자동 갱신.
- `tests/offline/test_graph_export.py`: build_report 매핑/빈입력/쿼리스트링 + focus 미리계산 4건.

## P3a 상태 — Run 실행 구조(레벨 병렬) 정적 뷰 (2026-06-15)
- 정적 `run.html`: 합성 plan을 **실행 단계(level)** 로 분해 — 같은 깊이=병렬 후보, 단계 간 배리어.
  단계별 동시 노드·추정 소요 + 순차 vs **레벨 병렬 wall-clock** + 가속(모델 `ready.timeout` 기반).
- `build_catalog` 노드 메타에 `ready_timeout` 추가(추정용). catalog/plan/report/run 상호 네비.
- **라이브 상태 오버레이는 서버 후속**: control plane `/runs/{id}/graph`가 oplog 이벤트를
  같은 그래프에 색칠(생성중/완료/실패). 정적 run.html은 구조/추정까지.

> **정적 4종(Catalog·Plan·Run·Report) + 라이브 그래프(form/compose) 착륙 완료.**
> 남은 서버 작업: ① `/runs/{id}/graph` oplog 상태 오버레이(3a-live) ② **P3b 엔진 레벨 병렬
> executor**(barrier-group emit + quota 동시성) — 별도 트랙. 렌더 Cytoscape 업그레이드는 옵션.
