...
└──────────────┬────────────────────────────────────────────┬─────────────┘
               │ dispatch / 명령                  하트비트 / 결과 │
┌──────────────▼────────────────── Execution Plane (기존) ───▼─────────────┐
│  GitHub Actions runner  ─또는─  전용 서버 worker (Phase 3)                │
...
2. **모든 설정은 데이터.** 환경 프로파일, 스위트, 스케줄, knowledge —
   전부 DB/파일의 선언적 레코드. UI는 그 위의 편집기일 뿐, git에 있는
   `scenarios.json`·`knowledge/formal/`이 계속 source of truth (UI 편집 →
   브랜치 커밋 → validator 통과 → 머지 흐름).
3. **secret은 플랫폼이 저장하지 않는다.** 환경 프로파일은 credential의
   *참조*(GitHub Secrets 이름 또는 vault 키)만 보관.
...
- **수동 실행**: UI에서 suite·환경·옵션(crud_filter, run_heavy…) 골라
  실행 — 내부적으로 `workflow_dispatch` API 호출 (Phase 3에선 worker에
  직접 큐잉). 현재 `.github/run-request` 파일 편집 방식을 대체.
...
| 시나리오 skip / run abort | **명령 채널**: 엔진이 step 경계마다 플랫폼에서 pending 명령 폴링 (`engine.py`에 체크포인트 1곳 추가). Actions의 cancel보다 정밀 — abort 시에도 역순 cleanup은 수행 |
| 막힌 폴링 강제 타임아웃 | 같은 명령 채널 |

주의: GitHub Actions 위에서는 명령 채널이 "플랫폼 API를 엔진이 폴링"으로
동작해야 함 (runner에 인바운드 불가). 전용 worker로 가면 그대로 유지되므로
설계 변경 없음.
...
| DB | **SQLite → PostgreSQL** | runs/schedules/environments는 소규모. 단일 서버 SQLite로 시작, 필요 시 이관 |
| 프론트 | 서버 렌더 + **htmx** (또는 React, 팀 선호 시) | 기존 dashboard/build.py HTML 자산 재활용, 별도 빌드체인 최소화 |
| 실행기 | **Phase A: GitHub Actions 유지** (dispatch API로 제어) → **Phase B: 전용 worker** | ROADMAP Phase 3과 동일 경로. 개입 정밀도·VPN 접근이 필요해지는 시점에 worker 전환 |
| 결과 저장 | 기존 JSONL 유지 + run별 S3 보관 | 스키마 변경 없음 — `dashboard.build`가 이미 JSONL만 읽음 |
...
- [ ] FastAPI 서버 + DB (environments / suites / schedules / runs)
...
### M3 — 저작도구 + AI 파이프라인 완성
- [ ] 시나리오/스위트 편집 UI (스키마 검증 + 할당량 시뮬레이션 + git 커밋 플로우)
...
- [ ] AI A1(spec-diff 분석→부분 재실행 제안)·A2(시나리오 초안)·A3(fact
      추출) 파이프라인 + C1 탐색 모드
- [ ] (선택) 전용 worker 전환 — ROADMAP Phase 3 합류

### 마일스톤별 가치
- M0만으로도: 멀티 환경 매트릭스 + run 히스토리가 생김 (서버 없이).
- M1로: "스케줄에 따라 수행 + 히스토리 리포팅" 요구 충족.
- M2로: "콘솔 로그인 없이 조치" 요구 충족.
- M3로: "저작도구 + AI 상시 파이프라인" 완성 — 플랫폼 완결.

---

## 6. 열린 결정 사항

1. **프론트엔드 스택** — htmx 서버렌더(권고: 가볍고 기존 HTML 재활용) vs
   React SPA(팀 표준이면). M1 시작 전 결정.
2. **플랫폼 서버 호스팅 위치** — 검증계 내부 VM(권고: VPN 뒤 서비스 접근,
   Phase 3 worker와 동거) vs 외부. credential 참조 방식이 이 결정에 종속.
3. **UI 편집의 git 반영 방식** — 직접 커밋 vs PR 생성(권고: 시나리오·
   knowledge는 PR + validator CI, 스케줄·환경 같은 운영 설정은 DB 직접).
