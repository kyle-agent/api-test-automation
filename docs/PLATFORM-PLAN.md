...
└──────────────┬────────────────────────────────────────────┬─────────────┘
               │ dispatch / 명령                  하트비트 / 결과 │
┌──────────────▼────────────────── Execution Plane (기존) ───▼─────────────┐
│  동일 호스트 worker (기본)  ─또는─  GitHub Actions runner (보조)           │
...
2. **모든 설정은 데이터, source of truth는 호스트의 파일.** 환경 프로파일,
   스위트, 시나리오, knowledge — 전부 플랫폼 호스트의 repo working copy에
   있는 선언적 파일(`scenarios.json`·`knowledge/formal/`·`suites/`·
   `environments/`). **UI 편집 = 이 파일들의 직접 변경** (저장 시 validator
   게이트). worker가 같은 디렉토리를 읽으므로 편집이 즉시 다음 run에 반영.
   git은 이력·백업·되돌리기 용도 (편집마다 자동 로컬 커밋, 원격 push는
   운영자 재량).
...
4. **regression 핫패스는 결정적으로 유지.** AI는 저작·후처리·옵트인 탐색에
   배치한다 (§4).
5. **호스트 불문 동일 배포.** AWS·로컬 PC·내부 VM 어디든 같은 형태로 뜨도록
   Docker Compose 한 벌(server + worker + 공유 repo 볼륨)로 패키징, DB는
   파일 기반 SQLite. 호스트 이전 = 디렉토리(repo + DB + 스냅샷) 이동.
...
- **수동 실행**: UI에서 suite·환경·옵션(crud_filter, run_heavy…) 골라
  실행 — run 레코드 생성 → 동일 호스트 worker가 큐에서 소비. 현재
  `.github/run-request` 파일 편집 방식을 대체. (GitHub Actions를 보조
  실행기로 쓸 경우에만 `workflow_dispatch` 호출 — 단, 이때는 호스트 파일
  편집분이 git push되어 있어야 러너에 반영됨에 유의.)
...
worker가 플랫폼 서버와 동일 호스트이므로 명령 채널은 DB(또는 로컬 파일)
폴링으로 단순하게 구현. 보조 실행기인 GitHub Actions에서 돌 때만 플랫폼
API를 HTTP로 폴링 (runner에 인바운드 불가) — 엔진 쪽 인터페이스는 동일.
...
| 호스팅 | **호스트 불문** (AWS·로컬 PC·내부 VM) — Docker Compose 패키징 (확정) | server + worker + 공유 repo 볼륨 한 벌. 어디서 떠도 동일 동작, 이전 = 디렉토리 이동 |
| DB | **SQLite** (파일 기반) | 호스트 불문 요구에 부합 — 외부 DB 의존 없음. runs/schedules 규모상 충분 |
| 실행기 | **동일 호스트 worker (기본)**, GitHub Actions는 보조 | UI 편집 = 호스트 파일 변경이므로 같은 파일을 읽는 worker가 자연스러움 (ROADMAP Phase 3 Step 2를 앞당김). Actions 경로는 push된 상태에서만 유효 |
...
### M3 — 저작도구 + AI 파이프라인 완성
- [ ] 시나리오/스위트 편집 UI — §3.1 파일 기반 편집 모델 (검증 + 원자적
      쓰기 + 자동 로컬 커밋)
...
2. **호스팅** — 특정 위치에 묶지 않음 (AWS·로컬 PC·내부 VM 모두 가능) →
   ...
   파일/env 참조.
3. **UI 편집 반영** — 호스팅 위치의 **파일 직접 변경** (§3.1). git은
   이력·백업용 자동 로컬 커밋, 원격 push는 선택. 이 결정의 귀결로 기본
   실행기는 동일 호스트 worker (GitHub Actions는 push 후에만 유효한 보조).
