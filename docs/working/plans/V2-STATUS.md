...
1. v2 자체 발사(pre-flight 컨펌 모달) — **오너 검수 게이트 후 활성화** (§2.6).
2. L2 그래프 단일화(오버레이 4종) — 기존 세션 console2 작업과 충돌 주의,
   **시작 전 main pull**. 서비스 상세 의존 미니그래프(D6 인스펙터)도 이때.
3. L3 네비 전환 + 리다이렉트 — **G3 오너 게이트** (되돌리기 비쌈).
4. L4 legacy 흡수(ops.html 등) — 이식 검증 전 제거 금지.
5. 잔존 자원 수집 캐시(M5) — 현재 Overview KPI는 legacy 뷰 링크만.
