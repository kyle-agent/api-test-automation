...
1. **Runs(실행 축) v1** — 계약 §2.6 확정됨. 발사 없음(핸드오프), 실효 게이트
   표시, 모니터링(console_api), 기록. 구현 위임 중 → 페르소나 검수 → 커밋.
2. v2 자체 발사(pre-flight 컨펌) — **오너 검수 게이트 후 활성화** (§2.6).
3. Model 축 (모델 표·작업 큐·노드 에디터·인벤토리 — D6: 독립 의존그래프 없음).
4. L2 그래프 단일화 — 기존 세션 console2 작업과 충돌 주의, **시작 전 main pull**.
5. L3 네비 전환 + 리다이렉트 — **G3 오너 게이트** (되돌리기 비쌈).
6. L4 legacy 흡수 — 이식 검증 전 제거 금지.
