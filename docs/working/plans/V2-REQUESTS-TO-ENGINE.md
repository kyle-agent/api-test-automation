...
## #4 — 발행 산출물 간 판정 일관성 (수동 발행 케이스)

- **무엇**: 결과 없는 수동 발행(conformance refresh 등) 시 `endpoint_status.json.
  updated`(발행 시각)는 갱신되는데 `history.jsonl` 마지막 줄(판정)과 `index.html`
  헤드라인은 서로 다른 시점을 가리키게 됨 — 실측(2026-07-10): 발행 07-09는
  healthy 배너, history 마지막 줄은 07-07 `fail_new=2`.
- **왜**: 소비자마다 "최신 판정"이 달라짐. v2는 판정 배지에 history 행 ts를
  분리 표기하는 것으로 방어 중이나, 발행 시 판정 줄을 함께 이월(또는 배너를
  history 기준으로 생성)하면 근본 해소.

## #5 — VPC 세마포어 "슬롯 대기" 이벤트 발행

- **무엇**: `regression/scenarios/engine.py`의 VPC 세마포어(SCP_VPC_SEMAPHORE)
  가 슬롯 대기에 들어가고/벗어날 때 콘솔 이벤트(예: `semaphore-wait` /
  `semaphore-acquired`, lifecycle·경과 포함)를 SCP_CONSOLE_EVENTS로 발행.
- **왜**: v2 실행 뷰(계약 §2.9)의 "지연 의심" 휴리스틱이 세마포어 대기(정상
  조율)를 지연으로 오탐하지 않으려면 대기 상태를 구분할 신호가 필요. 이벤트가
  생기기 전까지 v2는 VPC-생성 스텝을 지연 판정에서 제외하는 보수적 우회 중.
