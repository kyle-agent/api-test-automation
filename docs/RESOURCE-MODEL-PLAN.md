...
| 단계 | 내용 | 산출물 | 상태 |
|---|---|---|---|
| **R1 모델+역추출** | 스키마 확정·validator 확장, 기존 128 lifecycle에서 body/capture/poll/teardown **역추출**해 networking부터 task화 (nw-vpc 묶음이 파일럿), cross-service.yaml requires는 모델로 흡수(당분간 양존) | `knowledge/formal/resources/` + 추출기 | **DONE** — 전 카테고리 fan-out 완료: **127 노드 / 50 파일 / 48 그룹** (12 카테고리), validator R1 검사 통합, provenance VALIDATED 56 / docs 71 |
| **R2 합성기** | composer + 동치 검증(합성 결과 vs 수작업 lifecycle diff), "이 자원만 테스트" UI | `composer.py`, Planning UI | **DONE** — 단위·묶음 합성 + credential requires·충돌-재시도 teardown·filter capture·delete body/PUT teardown·lookup 노드까지 지원, `gen-pilot-net-basics` 20/20 라이브 증명, `/planning/resources` 폼·합성 미리보기 가동 |
| **R3 전환** | 노드별 라이브 검증 통과 → 수작업 lifecycle을 합성본으로 교체(점진), 전체 회귀 plan 계산(adopt/lane 일반화) | scenarios.json 축소 | **진행 중** — 웨이브 1(3회전)·웨이브 2(3회전)·heavy 윈도우(2회전+rev 3 디스패치) 완료(§6), 합성 roster **안정 green ~10개**, 56 노드 VALIDATED(128 중 — lookup 노드 추가로 127→128). 수작업 lifecycle 교체·전체 회귀 plan은 미착수 |
| **R4 C4 변형** | `vary` 옵션 조합 변형 생성 + 커버리지 집계에 C4 축 추가 | 변형 suite | **대기** (R3 이후) |
...
## 6. 라이브 검증 결과 — R3 웨이브 (2026-06-12 현재)
...
- **콘솔-전용 id** — alert/event-rule(servicewatch)·cm-event-policy
  ...
  id를 옵션으로 입력**하는 경로가 정답 (§3). cm-event-policy는
  `productResourceId`만 API 발견 가능 — cm-account-resource lookup 노드 +
  X-ResourceType 헤더로 진전(헤더 수용 확인, run 27401527554)했으나 계정에
  모니터링 대상 VM이 없어 404 → heavy 윈도우에서 서버 생성 후 재시도.
