...
| **R3 전환** | 노드별 라이브 검증 통과 → 수작업 lifecycle을 합성본으로 교체(점진), 전체 회귀 plan 계산(adopt/lane 일반화) | scenarios.json 축소 | **진행 중** — 검증 웨이브 1이 3회전 완료(§6), ~56 노드 VALIDATED. 수작업 lifecycle 교체·전체 회귀 plan은 미착수 |
...
| `gen-wave-dashboard` | create 201 증명, capture를 `$.id`(flat)로 수정해 rev 4 재합성 | 27396649009 |

웨이브 1 triage 3회전의 모델 수정·VALIDATED 승격으로 **~56 노드가
VALIDATED** (127 중; 나머지 71은 docs). fail_new 0 정책은 웨이브 내내 유지.
...
잔여 docs 노드 71개의 웨이브 fan-out 계속 → VALIDATED 승격 → 수작업
