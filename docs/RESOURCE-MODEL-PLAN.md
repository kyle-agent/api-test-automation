...
> 모델을 입력하는 것으로 추가된다.
>
> **문서 성격 (2026-07-04 정비):** 이 문서는 스키마·합성기·계약의 **설계
> 정본**이다. 본문 §0·§4의 노드/진행 수치는 작성 당시(06월 중순) 값 — 현재
> 수치는 `python -m tools.catalog_status`로 재측정 (모델은 275노드/60파일로
> 성장). 웨이브 라이브 로그는 §6 포인터의 트래커로 이관됐다.
...
>
> 살아있는 단일 출처들:
> - 노드별 검증 상태 — 각 `knowledge/formal/resources/*.yaml`의
>   `provenance`/`notes` (롤업: `python -m tools.catalog_status`)
> - 제품 결함 원장 — `docs/working/trackers/PRODUCT-FINDINGS.md`
> - 현행 커버리지 캠페인 — `docs/working/plans/CAMPAIGN-C3-100.md`
