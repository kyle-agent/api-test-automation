#!/usr/bin/env python3
"""Build a machine-readable catalog of all SCP API Reference endpoints.
...
Output: framework/api_catalog.json
"""
...
def main() -> int:
    ...
    endpoints = discover_endpoints(get_index_html())
    print(f"discovered {len(endpoints)} endpoints; {len(existing)} already in catalog", flush=True)
    ...
    for i, entry in enumerate(endpoints, 1):
        ...
        if cached and cached.get("method") and cached.get("http_path"):
            ...
            try:
                ...
                entry["error"] = str(exc)
                print(f"  [{i}/{len(endpoints)}] FAIL {entry['key']}: {exc}", flush=True)
        ...
        if done % 25 == 0:
            ...
            print(f"  progress {i}/{len(endpoints)} (resolved={ok})", flush=True)
